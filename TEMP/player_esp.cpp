...
void playeresp::draw_name(player_t* m_entity, const Box& box)
{
	...
	static c_menu::MoveStruct* MV_Name = &c_menu::get().MVItemList[c_menu::MI_NAME_TEXT];


	auto ChangeSidePosText = [](c_menu::MoveStruct* Curt, Vector2D BoxPos, int boxSizeX, int boxSizeY) -> Vector2D
	{
		switch (Curt->Side)
		{
		case c_menu::LEFT_S: return Vector2D(BoxPos.x - Curt->CorrectPadding.x, BoxPos.y + Curt->CorrectPadding.y);
		case c_menu::RIGHT_S:return Vector2D(BoxPos.x + boxSizeX + Curt->CorrectPadding.x, BoxPos.y + Curt->CorrectPadding.y);
		case c_menu::TOP_S:  return Vector2D(BoxPos.x + (boxSizeX / 2), BoxPos.y - Curt->CorrectPadding.y);
		case c_menu::DOWN_S: return Vector2D(BoxPos.x + (boxSizeX / 2), BoxPos.y + boxSizeY + Curt->CorrectPadding.y);
		case c_menu::LEFT_TOP_S:  return Vector2D(BoxPos.x, BoxPos.y - Curt->CorrectPadding.y);
		case c_menu::RIGHT_TOP_S: return Vector2D(BoxPos.x + boxSizeX, BoxPos.y - Curt->CorrectPadding.y);
		case c_menu::LEFT_DOWN_S: return Vector2D(BoxPos.x, BoxPos.y + boxSizeY + Curt->CorrectPadding.y);
		case c_menu::RIGHT_DOWN_S:return Vector2D(BoxPos.x + boxSizeX, BoxPos.y + boxSizeY + Curt->CorrectPadding.y);
		case c_menu::CENTER_LEFT_S:    return Vector2D(BoxPos.x - Curt->CorrectPadding.x, BoxPos.y + (boxSizeY / 2 + Curt->CorrectPadding.y));
		case c_menu::CENTER_RIGHT_S:   return Vector2D(BoxPos.x + boxSizeX + Curt->CorrectPadding.x, BoxPos.y + (boxSizeY / 2) + Curt->CorrectPadding.y);
		case c_menu::DOWN_LEFT_DOWN_S: return Vector2D(BoxPos.x - Curt->CorrectPadding.x, BoxPos.y + boxSizeY + Curt->CorrectPadding.y);
		case c_menu::DOWN_RIGHT_DOWN_S:return Vector2D(BoxPos.x + boxSizeX + Curt->CorrectPadding.x, BoxPos.y + boxSizeY + Curt->CorrectPadding.y);
		default: return Vector2D(0, 0);
		}
	};
	Vector2D TextPos = ChangeSidePosText(MV_Name, Vector2D(box.x, box.y), box.w, box.h);
	...
	if (m_engine()->GetPlayerInfo(m_entity->EntIndex(), &player_info))
	{
		...
		render::get().text(fonts[NAME], TextPos.x, TextPos.y, color, HFONT_CENTERED_X, name.c_str());
	}
}
...
void playeresp::draw_flags(player_t* e, const Box& box)
{
	...
	if (g_cfg.player.type[type].flags[FLAGS_SCOPED])
	{
		...
		if (scoped)
		{
			...
			render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "SCOPE");
			_y += 8;
		}
	}
	...
	if (g_cfg.player.type[type].flags[FLAGS_FAKEDUCKING])
	{
		...
		if (animstate)
		{
			...
			if (fakeducking() && e->m_fFlags() & FL_ONGROUND && !animstate->m_bInHitGroundAnimation)
			{
				...
				render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "FAKEDUCK");
				_y += 8;
			}
		}
	}
